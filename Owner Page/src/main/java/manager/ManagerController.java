...
public  class ManagerController implements Initializable {
    ...
    @FXML
    private LineChart<String, Integer> revenueChart;
    @FXML
    private PieChart salaryChart;
    @FXML
    private Label managerName, totalWaiter, totalCashier, totalKitchen, totalManager, totalMenu, totalItems, totalOrders;
    @FXML
    ...
    private TextField waiterSearchField, cashierSearchField, kitchenSearchField, managerSearchField, menuSearchField,
            inventorySearchField, quantityField, totalTipsField, rateField, sectionField, dishCategoryField,
    ...
    @FXML
    private TableColumn<Staff, Integer> waiterAgeColumn, cashierAgeColumn, kitchenAgeColumn, managerAgeColumn;
    @FXML
    private TableColumn<Staff, String> waiterPhoneColumn, cashierPhoneColumn, kitchenPhoneColumn, managerPhoneColumn;
    @FXML
    private TableColumn<Staff, String> waiterSalaryColumn, cashierSalaryColumn, kitchenSalaryColumn, managerSalaryColumn;
    @FXML
    private TableColumn<Staff, String> waiterDateColumn, cashierDateColumn, kitchenDateColumn, managerDateColumn;
    @FXML
    private TableColumn<Staff, String> waiterShiftColumn, cashierShiftColumn, kitchenShiftColumn, managerShiftColumn;
    @FXML
    private TableColumn<Dish, String> menuNameColumn, menuCategoryColumn;
    @FXML
    private TableColumn menuDescriptionColumn, menuStatusColumn,
            orderIdColumn, orderDateColumn, orderDiscountColumn, orderTotalColumn, orderTotalPaidColumn, orderSourceColumn;
    @FXML
    private TableColumn<Dish, Double> menuPriceColumn, menuCostColumn;
    @FXML
    ...
    @FXML
    private TreeView<Section> sectionViewer;
    ...
    private final String[] orderChoices = {"ID", "Date", "Discount", "Total", "Total Paid", "Source"};
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        panes.addAll(Arrays.asList(waiterPane, cashierPane, kitchenPane, managerPane, menuPane, inventoryPane,
                qrCodePane, statPane, settingsPane, ordersPane));
    }
    ...
    public void addStaff(ActionEvent event){
        ...
        if(event.getSource() == addTable){
            ...
            tableController.openAddSection(sectionViewer.getSelectionModel().getSelectedItem().getValue());
            refreshSectionViewer();
        }
    ...
    public void remove(ActionEvent event) {
        ...
        if(event.getSource() == deleteSection){
            connection.deleteSectionOrTable(sectionViewer.getSelectionModel().getSelectedItem().getValue());
            refreshSectionViewer();
    ...
    public void switchPanes(ActionEvent event){
        ...
        if (event.getSource() == waiterButton) {
            ...
            targetButton = inventoryButton;
            targetIcon = inventoryIcon;
        } else if (event.getSource() == qrCodeButton) {
            targetPane = qrCodePane;
            targetButton = qrCodeButton;
            targetIcon = qrCodeIcon;
        } else if (event.getSource() == statButton) {
            targetPane = statPane;
            targetButton = statButton;
            targetIcon = statIcon;
        } else if (event.getSource() == settingsButton) {
            targetPane = settingsPane;
            targetButton = settingsButton;
            targetIcon = settingsIcon;
        } else if (event.getSource() == ordersButton){
            targetPane = ordersPane;
            targetButton = ordersButton;
            targetIcon = ordersIcon;
        }

        if(targetPane != null){
            if(!targetPane.isVisible()){
                changePane(targetPane, targetButton, targetIcon);
            }
        }
    }
    public void changePane(Pane selectedPane, Button selectedButton, FontIcon selectedIcon){
        ...
        for(int i = 0; i < buttons.size(); i++) {
            changeOnHover(buttons.get(i), icons.get(i));
        }
        selectedButton(selectedButton, selectedIcon);
    }
    ...
    public void openEditPane(ActionEvent event) {
        ...
        if(event.getSource() == editSection){
            if(!sectionField.getText().isEmpty()){
                connection.editSection(sectionViewer.getSelectionModel().getSelectedItem().getValue(), sectionField.getText());
                sectionViewer.getSelectionModel().clearSelection();
                refreshSectionViewer();
                sectionField.clear();
            }
        ...
        if(event.getSource() == editTable){
            TableManagerController tableController = new TableManagerController();
            tableController.setManagerController(this);
            tableController.openEditTablePane(sectionViewer.getSelectionModel().getSelectedItem().getValue(),
                    sectionViewer.getSelectionModel().getSelectedItem().getParent().getValue().getName());
            sectionViewer.getSelectionModel().clearSelection();
            refreshSectionViewer();
        }
    ...
    public void staffSearch(String search, ChoiceBox<String> choice, ObservableList<Staff> searchList, ObservableList<Staff> searchResult) {
        switch (choice.getValue()) {
            ...
            case "Salary":
                for (Staff staff : searchList) {
                    if (staff.getSalaryUSD() == Double.parseDouble(search) || staff.getSalaryLBP() == Double.parseDouble(search))
                        searchResult.add(staff);
                }
                break;
            case "Phone Number":
                for (Staff staff : searchList) {
                    if (staff.getPhoneNumber().toUpperCase().equals(search))
                        searchResult.add(staff);
                }
                break;
            case "Date Joined":
                for (Staff staff : searchList) {
                    if (staff.getDateJoined().equals(search))
                        searchResult.add(staff);
                }
                break;
            case "Shift":
                ...
                for (Staff staff : searchList) {
                    if (staff.getShift().equals(search))
                        searchResult.add(staff);
                }
                break;
        }
...
//        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Directories","*"));
    ...
    public void openTipsDialog(ActionEvent event){
        StaffManagerController controller = new StaffManagerController();
        controller.setManagerController(this);
        controller.openTipsPane(connection.getTotalTips(), managers.size(), waiters.size(), cashiers.size(), kitchens.size());
    }
    /****** STATISTICS SECTION ******/
    ...
    public void initializeTreeViewSection(){
        ...
        sectionViewer.setShowRoot(false);
    ...
    public void refreshSectionViewer(){
        ...
        for(Section section : sections){
            rootItem.getChildren().add(section.getSectionNode());
        }
        ...
        if(event.getSource() == sectionField){
            if(sectionField.getText().isEmpty()){
                ...
                if(sectionViewer.getSelectionModel().getSelectedItem() != null){
                    if(sectionViewer.getSelectionModel().getSelectedItem().getValue().getName() != null){
    ...
    public void initializeWaiterTable(){
        ...
        waiterSalaryColumn.setCellValueFactory(new PropertyValueFactory<Staff, String>("salaryExchanged"));
...
//        waiterTable.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE); // So We can select more than one
    ...
    public void initializeCashierTable(){
         ...
         cashierSalaryColumn.setCellValueFactory(new PropertyValueFactory<Staff, String>("salaryExchanged"));
    ...
    public void initializeKitchenTable(){
        ...
        kitchenSalaryColumn.setCellValueFactory(new PropertyValueFactory<Staff, String>("salaryExchanged"));
    ...
    public void initializeManagerTable(){
        ...
        managerSalaryColumn.setCellValueFactory(new PropertyValueFactory<Staff, String>("salaryExchanged"));
...
//                        });
    ...
    public void initializeCharts(){
        int totalWaiters = getWaitersTotalSalaries();
        int totalCashiers = getCashierTotalSalaries();
        int totalKitchen = getKitchenTotalSalaries();
        int totalManagers = getManagerTotalSalaries();
        int totalSalaries = totalWaiters + totalCashiers + totalManagers + totalKitchen;

        ObservableList<PieChart.Data> salaryData = FXCollections.observableArrayList(
                new PieChart.Data("Managers\n$" + formatter.format(totalManagers) + " / LBP" +
                        formatter.format(totalManagers * rate), totalManagers),
                new PieChart.Data("Waiters\n$" + formatter.format(totalWaiters) + " / LBP" +
                        formatter.format(totalWaiters * rate), totalWaiters),
                new PieChart.Data("Cashiers\n$" + formatter.format(totalCashiers) + " / LBP" +
                        formatter.format(totalCashiers * rate), totalCashiers),
                new PieChart.Data("Kitchen Staff\n$" + formatter.format(totalKitchen) + " / LBP" +
                        formatter.format(totalKitchen * rate), totalKitchen)
        );
        salaryChart.getData().addAll(salaryData);
        salaryChart.setTitle("Total Salaries: $ " + totalSalaries);
        salaryChart.setLegendVisible(false);

        XYChart.Series<String, Integer> revenueData = new XYChart.Series<>();
        revenueData.setName("Revenue");
        revenueData.getData().add(new XYChart.Data<>("May", 52360));
        revenueData.getData().add(new XYChart.Data<>("June", 78062));
        revenueData.getData().add(new XYChart.Data<>("July", 40000));
        revenueData.getData().add(new XYChart.Data<>("May1", 52360));
        revenueData.getData().add(new XYChart.Data<>("June1", 78062));
        revenueData.getData().add(new XYChart.Data<>("July1", 40000));
        revenueChart.getData().add(revenueData);
        revenueChart.setLegendVisible(false);
        // Shows Value of Data on hover
        for(XYChart.Series<String, Integer> s : revenueChart.getData()){
    ...
    public void initializeTips(){
        totalTips = 2457.69;
        totalTipsField.setText(connection.getTotalTips().toString());
    }
    ...
    public void initializeSideBar(){
        ...
        selectedButton(waiterButton, waiterIcon);
        changeOnHover(cashierButton, cashierIcon);
        changeOnHover(kitchenButton, kitchenIcon);
        changeOnHover(managerButton, managerIcon);
        changeOnHover(ordersButton, ordersIcon);
        changeOnHover(statButton, statIcon);
        changeOnHover(menuButton, menuIcon);
        changeOnHover(inventoryButton, inventoryIcon);
        changeOnHover(qrCodeButton, qrCodeIcon);
        changeOnHover(settingsButton, settingsIcon);
        changeOnHover(logoutButton, logoutIcon);
...
//                }
    ...
    /***** TOTALS CALCULATION *****/
    public int getWaitersTotalSalaries() {
        int totalSalaries = 0;
        for (Staff waiter : waiters) {
            totalSalaries += waiter.getSalary();
        }
        totalWaiter.setText(formatter.format(totalSalaries));
        return totalSalaries;
    }
    public int getCashierTotalSalaries() {
        int totalSalaries = 0;
        for (Staff cashier : cashiers) {
            totalSalaries += cashier.getSalary();
        }
        totalCashier.setText(formatter.format(totalSalaries));
        return totalSalaries;
    }
    public int getKitchenTotalSalaries() {
        int totalSalaries = 0;
        for (Staff kitchen : kitchens) {
            totalSalaries += kitchen.getSalary();
        }
        totalKitchen.setText(formatter.format(totalSalaries));
        return totalSalaries;
    }
    public int getManagerTotalSalaries() {
        int totalSalaries = 0;
        for (Staff manager : managers) {
            totalSalaries += manager.getSalary();
        }
        totalManager.setText(formatter.format(totalSalaries));
        return totalSalaries;
    }
    ...
    private void changeOnHover(Button button, FontIcon icon) {
        if(!button.getText().equals("")){
            button.setStyle("-fx-border-color: #fefefe;" +
                    "-fx-text-fill: #fefefe;");
            ...
            button.setOnMouseEntered(event ->{icon.setIconColor(darkBlue);});
            button.setOnMouseExited(event ->{icon.setIconColor(white);});
        }
    }

    public void selectedButton(Button button, FontIcon icon) {
        if(!button.getText().equals("")){
            button.setStyle("-fx-border-color: #363753;" +
                    "-fx-text-fill: #363753;");
        }
